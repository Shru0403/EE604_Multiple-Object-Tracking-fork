...
class ByteTrack:
    ...
    def update_tracks(self, detections: torch.Tensor, format: str="xywh") -> List[Track]:
        ...
        if len(DHigh_ious) > 0:
            ...
            # DHigh_match_indexes, _ = linear_sum_assignment(1.0-DHigh_ious, maximize=False)
            row_ind, col_ind = linear_sum_assignment(1.0-DHigh_ious.cpu().numpy(), maximize=False)
            
            # Convert NumPy indices to PyTorch Tensor and stack them
            DHigh_match_indexes = torch.stack([
                torch.from_numpy(row_ind).to(device), 
        ...
        if len(DLow_ious) > 0:
            ...
            # DLow_match_indexes, _ = linear_sum_assignment(1.0-DLow_ious, maximize=False)
            cost_matrix = 1.0 - DLow_ious.cpu().numpy()
            
            # 2. Run assignment, getting 1D arrays for rows and columns
            row_ind, col_ind = linear_sum_assignment(cost_matrix, maximize=False)
            
            # 3. Convert NumPy indices back to PyTorch Tensor and stack into N x 2 matrix
            DLow_match_indexes = torch.stack([
                torch.from_numpy(row_ind).to(device), 
                torch.from_numpy(col_ind).to(device)
            ], dim=1)
